...
class PipelineAsset():
  ...
  def __init__(self, key: str, context: PipelineContext, df: DataFrame=None, db: str=None, 
               base_name: str=None, table: str = None, cache: bool = False):
    '''
    key: string key that names this asset.
    context: Current PipelineContext object for the pipeline.
    df (optional): The dataframe containing the data.
    db (optional): The database name, if given, along with the base_name, the data will be read from 
    that location on disk and the dataframe will be populated.
    base_name (optional): The table base name, if given, along with the db, the data will be read from 
    ...
    table (option): Works the same as base_name but the current date, git version, and params version are 
    not added to the name before reading/writing. An error is raised if both base_name and table are set.
    cache (optional, default=False): Cache the dataframe and unpersist when this object goes out of scope.
    '''
    self._key = key
...
class PipelineStage(ABC):
  ...
  def run(self, context, log, limit: bool = False):
    ...
    if self.name == "CreatePatientCohortTableStage":
