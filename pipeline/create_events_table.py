...
class CreateEventsTableStage(PipelineStage):
    ...
    def _run(self, context, log):
        ...
        ## UNIQUE VALUE CHECK
        self._check_unique_identifiers_events()

        log._timer(self.name, end=True)

        return {
            self._events_table_output: PipelineAsset(
                key = self._events_table_output,
                context = context,
                db = params.DATABASE_NAME,
                df = self._events_data,
                cache = False,
                delta_table = True,
        ...
        _data_entries: List[EventsStageDataEntry] = [
            ...
            EventsStageDataEntry(
                ...
                mapping = EventsStageColumnMapping(
                    ...
                    record_id_field = params.CVDP_COHORT_PRIMARY_KEY,
            ...
            EventsStageDataEntry(
                ...
                mapping = EventsStageColumnMapping(
                    ...
                    category_field = F.lit(params.EVENTS_HES_APC_EPISODE_CATEGORY),
            ...
            EventsStageDataEntry(
                ...
                mapping = EventsStageColumnMapping(
                    ...
                    sex_field = params.SEX_FIELD,
                    dataset_field = params.DATASET_FIELD,
                    category_field = F.lit(params.EVENTS_HES_APC_SPELL_CATEGORY),
                    record_id_field = F.concat(F.lit(params.EVENTS_HES_APC_SPELL_PREFIX),F.col(params.HES_SPELL_ID_FIELD)),
            ...
            EventsStageDataEntry(
                ...
                mapping = EventsStageColumnMapping(
                    ...
                    flag_array_field = None,
                    assoc_record_id_field = F.array(F.col(params.ASSOC_REC_ID_FIELD))
                )),
    ...
    def _transform_data_to_schema_and_combine(self, context):
        ...
        for data_entry in self._data_entries:
            ...
            if data_entry.dataset_name.startswith('hes_apc'):
                print(f'[INFO] PROCESSING HES APC EVENTS DATAFRAME...')
    ...
    # Check unique identifiers in primary key column
    def _check_unique_identifiers_events(self):
        hash_check = check_unique_values(
            df = self._events_data,
            field_name = params.RECORD_ID_FIELD)
        if hash_check == True:
            pass
        else:
            raise Exception(f'ERROR: Non-unique hash values in Events Table. Pipeline stopped, check hashable fields.')